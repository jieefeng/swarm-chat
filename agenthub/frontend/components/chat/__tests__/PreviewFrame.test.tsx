...
describe("PreviewFrame", () => {
  it("渲染 iframe", () => {
    render(<PreviewFrame htmlCode="<div>Hello</div>" height={400} />);
  ...
  it("处理 HTML 代码", () => {
    render(<PreviewFrame htmlCode="<div>Test</div>" height={300} />);
    const iframe = screen.getByTitle("网页预览") as HTMLIFrameElement;
  ...
  it("默认高度为 400", () => {
    const { container } = render(<PreviewFrame htmlCode="<div>Hello</div>" />);
    const wrapper = container.firstChild as HTMLElement;
